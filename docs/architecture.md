...
Rub is a Python rewrite of [uxc](https://github.com/holon-run/uxc) (54K LOC Rust CLI) with a plugin-based adapter architecture. It provides a single CLI interface to **discover**, **inspect**, and **invoke** API operations across any protocol — OpenAPI, GraphQL, gRPC, JSON-RPC, MCP — where each protocol is a separately installable satellite package.

**Core principle:** `rub` core ships with **zero adapters**. All protocol support is plugged in via `pip install rub-openapi`, `pip install rub-graphql`, etc. This keeps the core lean and allows independent versioning and release cycles for each protocol adapter.

### Key Design Goals

- **Uniform interface** — One CLI contract (`rub <url> -h`, `rub <url> <op> -h`, `rub <url> <op> args`) across all protocols.
...
### AdapterRegistry (`rub.discovery`)

The `AdapterRegistry` is responsible for finding and managing adapter instances:

1. **Entry-point scanning** — Reads `importlib.metadata.entry_points(group="rub.adapters")` to find all installed adapter packages.
...
### OutputEnvelope (`rub.envelope`)

Every Rub command produces an `OutputEnvelope` — a Pydantic model that guarantees a deterministic JSON shape:
...
- **CLI integration** — `rub cache stats` shows entry counts; `rub cache clear` wipes the database.
...
- **Profiles** — Named credential configurations stored in `~/.config/rub/profiles.toml`. Each profile has a `name`, `auth_type` (bearer, api_key, basic, oauth2), and a `SecretSource` (literal value or environment variable).
- **Bindings** — Host-glob-to-profile mappings stored in `~/.config/rub/bindings.toml`. When no `--credential` flag is given, `AuthBindings.match(url)` auto-resolves the correct profile.
...
### RubFramework (`rub.__init__`)

The central runtime that wires everything together:

1. Creates a `pluggy.PluginManager` and registers `RubHookSpecs`.
...
### Configuration (`rub.config`)

`RubSettings` uses `pydantic-settings` with environment variable support:

| Setting | Env Var | Default | Description |
|---|---|---|---|
| `config_dir` | `RUB_CONFIG_DIR` | `~/.config/rub` | Base configuration directory |
| `cache_dir` | `RUB_CACHE_DIR` | `None` (uses config_dir) | Override cache database location |
| `cache_enabled` | `RUB_CACHE_ENABLED` | `True` | Enable/disable schema caching |
| `cache_ttl` | `RUB_CACHE_TTL` | `3600` | Cache TTL in seconds |
| `log_level` | `RUB_LOG_LEVEL` | `WARNING` | Logging level |

## Data Flow

### The Discover → Inspect → Invoke Pipeline

Rub routes every CLI invocation through a single pipeline. The mode is determined by which arguments are present:

```
rub <url> -h              → Discover mode  (no operation, -h flag)
rub <url> <op> -h         → Inspect mode   (operation present, -h flag)
rub <url> <op> key=value  → Invoke mode    (operation + arguments)
```

```mermaid
flowchart TD
    CLI["CLI Entry Point<br/>rub &lt;url&gt; [op] [args]"] --> Parse["Parse Arguments<br/>(RubGroup routing)"]
    Parse --> Framework["RubFramework Init<br/>pluggy + AdapterRegistry"]
...
1. **CLI parsing** — `RubGroup` (a custom Click group) intercepts subcommand routing so that positional `<url>` and `<operation>` arguments don't collide with Typer subcommands like `auth` and `cache`.

2. **Framework init** — `RubFramework()` creates the pluggy `PluginManager`, registers hook specs, loads entry-point plugins, and builds the `AdapterRegistry`.
...
Rub's plugin system is built on two complementary mechanisms:

1. **Python entry points** — For adapter discovery. Packages declare an entry point in the `rub.adapters` group, and Rub finds them automatically at runtime via `importlib.metadata`.
...
When `RubFramework()` is initialized:

```
1. pluggy.PluginManager("rub") is created
2. RubHookSpecs are registered as the hook specification
3. importlib.metadata.entry_points(group="rub.adapters") is scanned
...
After `pip install rub-openapi`, Rub will automatically:
- Find the `openapi` entry point in the `rub.adapters` group
- Load `rub_openapi.OpenAPIAdapter`
...
```python
from rub import RubFramework

framework = RubFramework()
...
Rub defines six lifecycle hooks via pluggy. Each hook fires at a specific point in the pipeline and receives relevant context.
...
For a full invocation (`rub <url> <op> key=value`), the hooks fire in this order:
...
```
on_error(error)                      ← fires for any RubError
```

### Implementing a Hook

Use the `@hookimpl` decorator from `rub.hooks`:
...
Rub uses `pydantic-settings` which merges configuration from multiple sources (highest priority first):

1. **Environment variables** — Prefixed with `RUB_` (e.g. `RUB_CACHE_TTL=7200`)
2. **`.env` file** — Loaded from the current working directory
3. **Defaults** — Built-in defaults in `RubSettings`
...
Rub is a Python rewrite of uxc with a fundamentally different architecture:

| Aspect | uxc (Rust) | Rub (Python) |
|---|---|---|
...
- **Agent integration** — Python-native Rub integrates more naturally into Python-based agent frameworks and workflows.
...
Rub uses a six-class error hierarchy rooted at `RubError`:
...
Every `RubError` carries a `message` and optional `details` string. The CLI catches these and maps them to `OutputEnvelope.error()` with appropriate error codes (`NO_ADAPTER`, `DISCOVERY_FAILED`, `INSPECT_FAILED`, `EXECUTION_FAILED`, `AUTH_FAILED`).

The `on_error` hook fires for any `RubError`, allowing plugins to add logging, metrics, or alerting.
