# Plugin Guide: Building a Rub Adapter

This guide walks through building a complete Rub satellite adapter package from scratch. By the end, you'll have a working adapter that Rub discovers automatically on install.

We'll build **rub-openapi** as the running example — a package that handles OpenAPI/Swagger URLs.
...
```bash
mkdir -p rub-openapi/src/rub_openapi
cd rub-openapi
```
...
```bash
mkdir -p src/rub_openapi tests
touch src/rub_openapi/__init__.py
touch src/rub_openapi/adapter.py
...
This is the most critical file — it tells Rub about your adapter via the entry points mechanism.
...
name = "rub-openapi"
version = "0.1.0"
description = "OpenAPI/Swagger adapter for Rub"
readme = "README.md"
...
- **Group:** `rub.adapters` — Rub scans this group at runtime
- **Name:** `openapi` — A unique identifier for your adapter
- **Value:** `rub_openapi:OpenAPIAdapter` — The module path and class name to load

When a user runs `pip install rub-openapi`, this entry point is registered in the environment's metadata. Rub's `AdapterRegistry` finds it via `importlib.metadata.entry_points(group="rub.adapters")`.
...
```python
# src/rub_openapi/__init__.py
"""Rub adapter for OpenAPI / Swagger APIs."""

from rub_openapi.adapter import OpenAPIAdapter
...
Create `src/rub_openapi/adapter.py`. Your adapter must subclass `rub.adapter.Adapter` and implement all six abstract methods:

```python
# src/rub_openapi/adapter.py
...
class OpenAPIAdapter(Adapter):
    ...
    async def _fetch_schema(self, url: str) -> dict[str, Any]:
        ...
        if resp.json_body is None:
            from rub.errors import SchemaRetrievalError
...
| Method | When Rub Calls It | What to Return |
|---|---|---|
...
| `list_operations(url)` | `rub <url> -h` | A list of `Operation` models |
| `describe_operation(url, op_id)` | `rub <url> <op> -h` | An `OperationDetail` model |
| `execute(url, op_id, args)` | `rub <url> <op> key=value` | An `ExecutionResult` model |
...
from rub_openapi import OpenAPIAdapter
...
class TestDetection:
    async def test_can_handle_openapi(self, adapter: OpenAPIAdapter) -> None:
        ...
        with patch.object(adapter, "_schema_url", return_value="https://example.com"):
            with patch("rub_openapi.adapter.AsyncHTTPClient") as MockClient:
...
from rub_openapi.adapter import OpenAPIAdapter
from rub_openapi.hooks import OpenAPIHooks
...
After installing, verify Rub can find your adapter:
...
rub petstore3.swagger.io/api/v3 -h

# Inspect a specific operation
rub petstore3.swagger.io/api/v3 listPets -h

# Invoke an operation
rub petstore3.swagger.io/api/v3 getPet petId=1
```
...
```bash
pip install rub-openapi
# or
uv add rub-openapi
```

That's it — after install, `rub` automatically discovers and uses your adapter. No configuration files to edit, no registration calls to make.
...
keywords = ["rub", "openapi", "swagger", "api", "cli"]
classifiers = [
...
dependencies = [
    "rub>=0.1.0",           # Core framework
    "httpx>=0.27",          # HTTP client (shared with rub core)
]
...
[project.urls]
Homepage = "https://github.com/yourname/rub-openapi"
Repository = "https://github.com/yourname/rub-openapi"
Issues = "https://github.com/yourname/rub-openapi/issues"
...
- [ ] `pyproject.toml` has `[project.entry-points.'rub.adapters']` section
- [ ] Entry point value points to a class that subclasses `rub.adapter.Adapter`
...
- [ ] Entry point is visible via `importlib.metadata.entry_points(group="rub.adapters")`
- [ ] `rub <url> -h` works with your adapter installed
...
- Check the entry point is registered: `python -c "import importlib.metadata; print(list(importlib.metadata.entry_points(group='rub.adapters')))"`
- Verify the package is installed: `pip list | grep rub-`
- Make sure the class path in the entry point is correct (module:Class format)

### Detection Fails

- Run with `-v` or `-vv` to see debug logs: `rub <url> -h -vv`
...
- Make sure `rub` core is installed as a dependency
