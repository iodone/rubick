---
name: rub-skill-creator
description: Create protocol adapter packages for Rub. Use when defining a new protocol adapter and you need reusable templates, validation rules, and anti-pattern guidance based on Rub's adapter architecture.
---

# Rub Skill Creator

Use this skill to design and standardize protocol adapter packages built on top of `rub`.

## Prerequisites

- `rub` skill is available as the base execution contract.
...
7. Register via entry point in `pyproject.toml`:
   ...
   - `myprotocol = "rub_myprotocol.adapter:MyProtocolAdapter"`
8. Write `README.md` with usage examples
9. Test installation and verify `rub <url> -h` works
...
- Base execution and auth guidance:
  - `skills/rub/SKILL.md`
- Rub adapter interface:
  - `src/rub/adapter.py` in Rub repo
- OpenAPI adapter reference:
  - `src/rub/adapters/openapi/adapter.py` in Rub repo
