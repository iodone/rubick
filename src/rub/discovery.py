...
from rub.errors import ProtocolDetectionError


class AdapterRegistry:
    """Discovers adapters via entry points and runs protocol detection.

    Scans ``importlib.metadata.entry_points(group='rub.adapters')`` to find
    ...
    async def detect_protocol(self, url: str) -> Adapter:
        ...
        attempted_str = ", ".join(attempted) if attempted else "(none installed)"
        msg = (
            f"No adapter could handle URL: {url}\n"
            f"Attempted adapters: {attempted_str}"
        )
