...
from rub.errors import AuthError
...
def make_auth_headers(profile: Profile) -> dict[str, str]:
    ...
    """
    if profile.auth_type == AuthType.custom:
        # Custom auth: support both custom_headers and secret_source
        if profile.custom_headers:
            return profile.custom_headers
        # Fallback: use secret as Authorization header (supports --secret flag)
        if profile.secret_source:
            secret = profile.resolve_secret()
    ...
    if credential is not None:
        ...
        if profile is None:
            raise AuthError(
                ...
                details="Use 'rub auth list' to see available profiles.",
            )
        ...
        if session is not None:
            if session.is_expired():
                if (
                    ...
                    logger.warning(
                        "OAuth2 token expired for '%s' but no "
                        "refresh_token/endpoint available",
                        profile.name,
                    )
