...
class LiteralSecret(BaseModel):
    """A secret value stored directly.

    (encrypted at rest is the user's responsibility)
    """
...
class Profile(BaseModel):
    ...
    def resolve_secret(self) -> str:
        ...
        """
        if self.auth_type == AuthType.custom and self.secret_source is None:
            raise AuthError(
        ...
        if isinstance(src, EnvSecret):
            ...
            if val is None:
                raise AuthError(
                    ...
                    details=(
                        f"Profile '{self.name}' references env var "
                        f"'{src.key}' which is not defined"
                    ),
                )
            return val
...
_DEFAULT_CREDENTIALS_PATH = Path.home() / ".config" / "rub" / "credentials.json"


class Profiles:
    """Manages loading, saving, and querying credential profiles.

    Persists to ~/.config/rub/credentials.json by default.
    """
