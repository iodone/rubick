"""CLI subcommands for auth management: rub auth set/list/remove/bind."""
...
from rub.auth.profile import AuthType, EnvSecret, LiteralSecret, Profile, Profiles
...
def auth_set(
    ...
    source: LiteralSecret | EnvSecret
    if secret:
...
def auth_list() -> None:
    ...
    for p in profiles:
        source_info = (
            ...
            if p.secret_source is not None and hasattr(p.secret_source, "key")
            else "literal:***"
        )
...
def auth_bind(
    ...
    meta: list[str] | None = typer.Option(
        ...
        "--meta",
        "-m",
        help="Metadata key=value pairs (e.g., --meta region=chnbj)",
    ),
) -> None:
    ...
    if alias and "/" in alias:
        typer.echo(
            "Warning: Alias contains '/' which will be "
            "interpreted as a URL path separator.",
            err=True,
        )
        host_part = alias.split("/")[0]
        path_part = alias.split("/", 1)[1]
        typer.echo(
            f"  URL 'echo://{alias}' → " f"hostname='{host_part}', path='/{path_part}'",
            err=True,
        )
        typer.echo(
            "  Recommended: Use '-' or '.' instead "
            "(e.g., 'region-us' or 'region.us')",
            err=True,
        )
    ...
    if meta:
        ...
        for item in meta:
            if "=" not in item:
                typer.echo(
                    f"Error: Invalid meta format '{item}' (expected key=value)",
                    err=True,
                )
                raise typer.Exit(code=1)
    ...
    typer.echo(
        f"Bound '{host}' → '{credential}'{alias_info}{meta_info} (priority {priority})"
    )
