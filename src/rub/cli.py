...
from rub.auth.cli import auth_app
from rub.cache_cli import cache_app
from rub.envelope import OutputEnvelope
from rub.errors import RubError
from rub.formatter import emit_rich, get_console


class RubGroup(TyperGroup):
    ...
    def parse_args(self, ctx: click.Context, args: list[str]) -> list[str]:
        ...
        for i, arg in enumerate(args):
            ...
            if arg in cmds:
                ...
                # Click >= 8.2: use _protected_args to avoid deprecation
                existing = getattr(ctx, "_protected_args", None)
                if existing is None:
                    existing = ctx.args
                ctx._protected_args = subcmd_args + existing
                return result
            break
...
app = typer.Typer(
    ...
    "Usage:\n\n"
    "  rub <URL> -h                 Discover operations\n\n"
    "  rub <URL> <operation> -h     Inspect an operation\n\n"
    "  rub <URL> <operation> k=v    Invoke with arguments",
...
def main(
    ctx: typer.Context,
    url: str | None = typer.Argument(None, help="Target URL to discover/invoke"),
    operation: str | None = typer.Argument(None, help="Operation to inspect or invoke"),
    ...
    \b
    Examples:
      rub https://api.example.com -h              Discover operations
      rub https://api.example.com listUsers -h     Inspect operation
      rub https://api.example.com getUser id=42    Invoke with args
    """
...
async def _run_pipeline(
    ...
    *,
    ...
    from rub.auth import AuthBinding, resolve_auth_headers
    from rub.auth.binding import AuthBindings
    from rub.formatter import _is_tty
    ...
    original_url = url
    parsed = urlparse(url)
    alias_credential: str | None = None
    alias_binding: AuthBinding | None = None
    if parsed.hostname:
        bindings = AuthBindings()
        bindings.load()
        real_host = bindings.resolve_alias(parsed.hostname)
        if real_host:
            # Remember which binding was matched via alias, so that auth
            # resolution uses its credential instead of re-matching by hostname
            # (which picks the first binding when multiple share the same host).
            for binding in bindings.list_bindings():
                if binding.alias and binding.alias.lower() == parsed.hostname.lower():
                    alias_credential = binding.credential
    ...
    try:
        # Prefer: explicit -c flag > alias-resolved credential > host-based matching
        resolved_credential = credential or alias_credential
        auth_headers, profile, matched_binding = await resolve_auth_headers(
            execution_url, credential=resolved_credential
        )
    ...
    # Skip meta injection when inspecting (-h) to avoid polluting the
    # "has user args" check in run_pipeline.
    # When credential was resolved via alias, matched_binding is None (explicit
    # credential path), so fall back to the alias_binding for meta injection.
    effective_binding = matched_binding or alias_binding
    if effective_binding and effective_binding.meta and not api_help:
        for key, value in effective_binding.meta.items():
            if key not in args:
                args[key] = value
