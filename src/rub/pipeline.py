...
``rub`` (the multi-protocol CLI) and standalone adapter CLIs share.
...
from rub.envelope import Metadata, OutputEnvelope
    ...
    *,
    ...
    try:
        ...
        for p in detail.parameters:
            if p.default is not None and p.name not in args:
                args[p.name] = p.default

        missing = [
            p.name
            for p in detail.parameters
            if p.required and (p.name not in args or args[p.name] is None)
        ]
        if missing:
            return OutputEnvelope.error(
